...
//=============================================================================
void free_comport(ComPort* port)
{
  free(port);
}
//=============================================================================
int open_comport(ComPort* port, int num, unsigned long baud, int stopbits, int databits)//int parity, int flowctrl)
{
  int errors = 0;
  ...
  //Set port name
  printf("/dev/ttyUSB%d", num-1);
  sprintf(port->name,"%s%d", "/dev/ttyUSB", num-1);
  //sprintf(port->name,"%s%d", "/dev/ttyUSB", num-1);
...
//=============================================================================
int close_comport(ComPort* port)
{

  if (!is_comport_open(port))
  {
    #if TraceErrors
...
//=============================================================================
int send_comport(ComPort* port, char* data)
{
  #if WINDOWS
  DWORD wrote;
  #endif
...
//=============================================================================
int recv_comport(ComPort* port, char* data, int max_length)
{
...
//=============================================================================
int is_comport_open(ComPort* port)
{
  //---------------------------
  #if LINUX
  //---------------------------
